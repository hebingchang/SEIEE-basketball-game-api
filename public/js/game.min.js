...
function preload() {
    ...
    game.load.image('hoop', 'assets/images/hoop_seiee.png');
...
function create() {
    ...
    current_best_text = game.add.text(55, 250, '', { font: 'Arial', fontSize: '20px', fill: '#000', align: 'center' });// 230, 450
    current_best_score_text = game.add.text(165, 366, '', { font: 'Arial', fontSize: '40px', fill: '#00e6e6', align: 'center' }); // 300, 500
    current_all_text = game.add.text(108, 335, '', { font: 'Arial', fontSize: '20px', fill: '#000', align: 'center' }); // 300, 500
    ...
    current_all_text.text = '划动篮球将球投入篮筐！';
}
...
function update() {
    ...
    if (ball && ball.body.velocity.y > 0 && ball.body.y > 188 && !ball.isBelowHoop) {
        ...
        if (ball.body.x > 151 && ball.body.x < 249) {
            ...
            var display_score = current_score;
            $.post( "/api/score", { _token: csrf, score: display_score, sign: sha256_digest(csrf+"_is_t0_s1gn_"+display_score) })
                .done(function( data ) {
                    current_best_text.text = '你为电院师生运动会进了'+display_score+'个球！\n打败了'+data.beat+'%的玩家';
                    current_all_text.text = '所有用户总计进球数:';
                    current_best_score_text.text = data.all;
                    document.title = '我为电院师生运动会进了'+display_score+'个球！';
                });
            current_score = 0;
        }
